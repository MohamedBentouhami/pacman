...
/*
permet l'adffichage du quizz en allant chercher l'image images ainsi que les questions
*/
...
function afficher_quiz() {
 ...
 /* vérifie si le nombre de réponse donnée coorespond bien aux nombre de réponse attendus */
	...
	quizzes[quizId].data.forEach(function(div, i) {
		...
		if(quizId === "mer" || quizId === "jeux"){
			...
			if(quizId === "couples" || quizId === "webg2"){
				...
				let checked_values = $("input[name=r"+i+"]:checkbox:checked").map(function(){
			      return $(this).val();
			    }).get(); 
				...
				if(div.bonneReponses.length >= 2){
					...
					if(checked_values.length < 2){
						...
						let msg =  $('<span>');
						msg.text("Cette question nécessite au moins 2 réponses!");
						msg.css('color', 'red');
						$('#question-'+div.id).after('<br>').after(msg);
